...
public class ProcessDrawing extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        while(count<15) {
            for (int j = 0; j <2 ; j++) {
                for (int i = 0; i < threadLiftManager.lift[j].getListForDrawing().size() - 1; ) {
                    move(rectangles[j], threadLiftManager.lift[j].getListForDrawing().get(0), threadLiftManager.lift[j].getListForDrawing().get(1), j);
                    threadLiftManager.lift[j].getListForDrawing().remove(0);
                    count++;
                }
            }
        }
/*        move(rectangles[0],0,5,0);
  ...
  /*     int count=0;
       while (count<2){
           for (int i = 0; i <threadLiftManager.lift[0].getListForDrawing().size() ; i++) {
               move(rectangles[0],0,threadLiftManager.lift[0].getListForDrawing().get(0),0);
               threadLiftManager.lift[0].getListForDrawing().remove(0);
               count++;
           }
       }*/
